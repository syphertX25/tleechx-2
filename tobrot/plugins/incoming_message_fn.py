...
async def g_yt_playlist(client, message):
    ...
    if "youtube.com/playlist" in url:
        i_m_sefg = await message.reply_text(
            f"💀 Downloading for <a href='tg://user?id={usr_id}'>{message.from_user.first_name}</a>",
            parse_mode="html",
        )
...
async def rename_tg_file(client, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            if message_to_send != "":
                mention_req_user = (
                    f"Hey <a href='tg://user?id={usr_id}'>{uname}</a>, Your Task is Completed🙂.\n\n"
                )
                message_to_send = message_to_send = f"\nFile Name: <code>{str(file.name)}</code>\n\n<b>📦 𝙏𝙤𝙩𝙖𝙡 𝙁𝙞𝙡𝙚 𝙎𝙞𝙯𝙚: {file.total_length_string()}</b>\n\n" + message_to_send 
                message_to_send = message_to_send + "\n\n" + mention_req_user + f"<b>Join @spider_mirror For Mirroring Your Links And @spider_cloud For Updates.🙂</b>"
            else:
