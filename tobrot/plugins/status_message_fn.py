    ...
    while True:
        ...
        for file in downloads:
            ...
            if file.status == "active":
                ...
                msg += f"\n\n{msgg}"
                msg += f"\n\nAdded By: <a href='tg://user?id={usr_id}'>{message.from_user.first_name}</a>"
        ...
        ms_g = (
            ...
            f"<b>Total:</b> <code>{total}</code> <b>\nUsed:</b> <code>{used}</code> <b>\nFree:</b> <code>{free}</code>\n"
            f"<b>RAM:</b> <code>{ram}%</code> | <b>CPU:</b> <code>{cpu}%</code>\n"
            f"\n📌UPDATES : @spider_cloud\n📌Mirror Group: @spider_mirror\n"
        )
        if msg == "":
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            await i_m_s_e_g.edit_text(
                f"Download cancelled :\n<code>File Name: {name} \nFile Size: ({size})</code> by <a href='tg://user?id={message.from_user.id}'>{message.from_user.first_name}</a>"
            )
